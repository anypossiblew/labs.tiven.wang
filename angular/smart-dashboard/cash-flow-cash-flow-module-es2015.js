...
let CashFlowComponent = class CashFlowComponent {
    constructor(nbThemeService, themeService) {
    ...
    ngAfterViewInit() {
        this.nbThemeService.getJsTheme()
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["takeUntil"])(this.destroy$), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["delay"])(1), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["tap"])(config => this.themeService.changeKpiChart1(config, this.kpiLineChart1Options)), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["tap"])(config => this.themeService.changeKpiChart2(config, this.kpiLineChart2Options)))
            .subscribe(config => {
        });
    }
    ngOnDestroy() {
...
let ThemeService = class ThemeService {
    ...
    changeKpiChart1(config, chartOptions) {
        const eTheme = config.variables.kpi1;
        let options = {
            grid: {
                left: -40,
                right: -40
    ...
    changeKpiChart2(config, chartOptions) {
        const eTheme = config.variables.kpi2;
        let options = {
            grid: {
                left: -50,
                right: -50
    ...
    changeTimeChart(config, chartOptions) {
        const eTheme = config.variables.times;
        let options = {
            grid: {
                left: 20,
                right: 30,
        ...
        this._ngZone.runOutsideAngular(() => { chartOptions.next(options); });
    }
    changeBankChart(config, chartOptions) {
        const eTheme = config.variables.bankBars;
        let options = {
            series: [{
                    label: {
                        normal: {
    ...
    changeCurrencyChart(config, chartOptions) {
        const eTheme = config.variables.currencyPies;
        let options = {
            color: eTheme.color,
        };
        this._ngZone.runOutsideAngular(() => { chartOptions.next(options); });
